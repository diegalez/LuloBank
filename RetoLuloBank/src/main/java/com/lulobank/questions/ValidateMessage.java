...
import static com.lulobank.utils.Constants.RESPONSE;

public class ValidateMessage implements Question<String> {

    private final String jsonPath;

    public ValidateMessage(String jsonPath) {
        this.jsonPath = jsonPath;
    }

    @Override
    public String answeredBy(Actor actor) {
        if (CODE_404.equals(actor.recall(STATUS_CODE).toString()) || CODE_429.equals(actor.recall(STATUS_CODE).toString())) {
    ...
    public static ValidateMessage inTheResponse(String jsonPath) {
        return new ValidateMessage(jsonPath);
    }
}
