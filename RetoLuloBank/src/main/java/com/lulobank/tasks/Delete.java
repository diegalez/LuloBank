...
import static com.lulobank.utils.Constants.CONTENT_TYPE;
...
public class Delete implements Task {
    ...
    public <T extends Actor> void performAs(T actor) {
        Response response = SerenityRest.given().relaxedHTTPSValidation().baseUri(actor.recall(URI)).contentType(CONTENT_TYPE)
                .and().log().all().
                when().delete(SLASH + actor.recall(RESOURCE_API)).
                then().extract().response();
        actor.remember(RESPONSE, response.getBody().asString());
        actor.remember(STATUS_CODE,response.getStatusCode());
        Show.information(Delete.class, RESPONSE_PATH + response.getBody().asString());
        Show.information(Delete.class, STATUS_CODE_ + response.getStatusCode());
    }

    public static Delete employee() {
